...
/*
 * 测试阿帕奇给我们提供的DBUtils工具类中有关查询的方法：
 * 
 * 4、各种的Handler……
 *  
 * 3. 测试 ResultSetHandler 的 BeanListHandler 实现类:
 * BeanListHandler: 把结果集转为一个 Bean 的 List. 该 Bean的类型在创建 BeanListHandler 对象时传入:
 * 	new BeanListHandler<>(Customer.class)
 * 
 * 
 * 2. 测试QueryRunner 的 query 方法:
 * ResultSetHandler 参数的作用: query 方法的返回值直接取决于ResultSetHandler 的 hanlde(ResultSet rs) 是如何实现的. 
 * 实际上, 在QueryRunner 类的 query 方法中也是调用了 ResultSetHandler 的 handle()方法作为返回值的。
 * 
 * 
 * 1.测试QueryRunner类的Update方法，该方法可用于INSERT,UPDATE,DELETE： 
 * 具体步骤：
 * （1）创建QueryRunner的实现类
 * （2）使用Update方法	(Update方法时重载的，注意要用哪一个，有了方法之后，看入参需要什么，再去创建)
 */
 ...
 */
	...
	/**
	 * 3.测试QueryRunner类的Update方法：
	 * 该方法可用于INSERT,UPDATE,DELETE
	 * 
	 * 具体步骤：
	 * 1、创建QueryRunner的实现类
	 * 2、使用Update方法
	 * 	(Update方法时重载的，注意要用哪一个，有了方法之后，看入参需要什么，再去创建)
	 */
	@Test
	public void testQueryRunnerUpdata(){
		
		QueryRunner queryRunner = new QueryRunner();//线程安全的，所以可以作为成员变量
