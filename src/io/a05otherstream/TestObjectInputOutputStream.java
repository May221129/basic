...
/**
 * 1.想让某个类是可序列化的，则必须实现下面两个接口中的一个接口：
 * 	implements Serializable 或 implements Externalizable
 * 2.ObjectInputStream和ObjectOutputStream是处理流，需要传给它一个节点流：
 * 	如果要将对象序列化到内存中，那么该节点流可以使用ByteArrayInputStream和ByteArrayOutputStream。
 * 	如果要将对象序列化到磁盘中，那么该节点流可以使用FileInputStream和FileOutputStream。
 */
